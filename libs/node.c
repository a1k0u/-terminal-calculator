...
NODE* create_node(STR* var, int type) {
    ...
    var = delete_symbols(var, 'j');
    if (var->data[strlen(var->data) - 1] == '.')
        var = delete_symbols(var, '.');
    ...
    node->type = type;
    node->number = CMPLXL(real, image);
    node->action = var->data[0];
...
NODES_ARRAY* insert_node_array(NODES_ARRAY* nodes_arr, NODE* node, int border) {
    NODES_ARRAY* new_arr = init_nodes_arr();
